...
def extract_selectors_from_template(template: str, /) -> Sequence[VariableSelector]:
    ...
    return [
        VariableSelector(variable=part, value_selector=part[1:-1].split("."))
...
class VariableTemplateParser:
    ...
    3. The template variable Key cannot contain new lines or spaces,
       and must comply with rule 2.
    ...
    # Output: [VariableSelector(
    #   variable='#node_id.query.name#', value_selector=['node_id', 'query', 'name']),
    #          VariableSelector(
    #            variable='#node_id.query.age#',
    #            value_selector=['node_id', 'query', 'age'])]
