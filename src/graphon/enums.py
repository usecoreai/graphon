...
class NodeExecutionType(StrEnum):
    ...
    # Nodes that can choose different branches (if-else, question-classifier).
    BRANCH = "branch"
    CONTAINER = (
        "container"  # Container nodes that manage subgraphs (iteration, loop, graph)
    )
...
class WorkflowExecutionStatus(StrEnum):
    ...
    # running can transition to succeeded, failed, or partial_succeeded
    # paused states can resume and return to running
    # stopped and terminal states transition to end (*)
    #
    # Mermaid diagram:
    #
    #     ---
    ...
    #         some execution occurred and was handled during execution
    #
...
class WorkflowNodeExecutionMetadataKey(StrEnum):
    ...
    ITERATION_DURATION_MAP = (
        "iteration_duration_map"  # single iteration duration if iteration node runs
    )
